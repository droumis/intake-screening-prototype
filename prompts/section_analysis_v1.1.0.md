# Section Analysis Prompt — v1.1.0
...
- Green flags: only emit 1-2 genuinely significant protective factors per section (strong support, relevant experience, clinical insight). The absence of a problem ("no allergies", "no mobility needs") is NOT a green flag.
- **Basis and citation:** Cite the criterion that produced each flag. When both a regulatory and a house criterion apply to the same fact, emit the regulatory citation as primary and note the house criterion in the rationale — never blend the bases. Include `basis` ("regulatory" or "house") and `citation` (the rule reference) in each flag.
- **Resolution criteria for pathway criteria:** When a criterion defines a documented resolution pathway (referral / clearance / consultation and risk review / written safety plan), name that pathway in `resolution_criteria` — not generic "follow up."
